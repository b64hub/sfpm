...
export class ApexAstSerializer {
  ...
  private resolveBinaryPath(): null | string {
  ...
  private async tryBinary(sourceCode: string, anonymous: boolean): Promise<null | RawAstResponse> {
  ...
  private async tryServer(sourceCode: string, anonymous: boolean): Promise<null | RawAstResponse> {
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 500);

      // eslint-disable-next-line n/no-unsupported-features/node-builtins -- fetch available in Node 18+ at runtime
      const response = await fetch(this.serverUrl, {
        body: JSON.stringify({anonymous, prettyPrint: false, sourceCode}),
