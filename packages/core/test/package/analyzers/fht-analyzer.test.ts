...
import FHTAnalyzer from '../../../src/package/analyzers/fht-analyzer.js';
...
describe('FHTAnalyzer', () => {
  let analyzer: FHTAnalyzer;
  ...
  beforeEach(() => {
    ...
    analyzer = new FHTAnalyzer(mockLogger);
  });
  ...
  describe('analyze', () => {
    it('should return empty when package has no custom fields', async () => {
      ...
      expect(pkg.setFhtFields).not.toHaveBeenCalled();
    });

    it('should detect fields with trackHistory enabled', async () => {
      const fields = [
      ...
      expect(pkg.setFhtFields).toHaveBeenCalledWith([
        'Account.MyField__c',
        'Contact.TrackedField__c',
      ]);
    });

    it('should set empty array when no fields have history tracking', async () => {
      const fields = [
