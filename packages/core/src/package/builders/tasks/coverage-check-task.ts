...
export default class CoverageCheck implements BuildTask {
    ...
    public async exec(): Promise<void> {
        ...
        if (!package2Version.HasPassedCodeCoverageCheck) {
            throw new Error(
                `Package version ${this.sfpmPackage.packageName}@${this.sfpmPackage.version} has not passed the code coverage check.` +
                ` It only has ${package2Version.CodeCoverage.apexCodeCoveragePercentage}% coverage`,
            );
        }

        this.logger.info(
            `Package version ${this.sfpmPackage.packageName}@${this.sfpmPackage.version} has passed the code coverage check.` +
