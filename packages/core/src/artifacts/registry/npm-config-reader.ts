...
export async function readNpmConfig(
  ...
  try {
    ...
    logger?.debug(`Failed to read npm config: ${error instanceof Error ? error.message : String(error)}`);
...
function extractScope(packageName: string): string | undefined {
  if (packageName.startsWith('@')) {
    ...
    if (slashIndex > 0) {
      return packageName.slice(0, Math.max(0, slashIndex));
    }
  }
...
function expandEnvVars(value: string): string {
  ...
  // Handle ${VAR} format
  let expanded = value.replaceAll(/\$\{([^}]+)\}/g, (_, varName) => process.env[varName] || '');

  // Handle $VAR format (only if not already expanded)
  expanded = expanded.replaceAll(/\$([A-Z_][A-Z0-9_]*)/gi, (_, varName) => process.env[varName] || '');
...
export function readNpmConfigSync(
  ...
  try {
    ...
    logger?.debug(`Failed to read npm config (sync): ${error instanceof Error ? error.message : String(error)}`);
