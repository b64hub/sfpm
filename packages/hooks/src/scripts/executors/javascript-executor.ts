...
/**
 * Executes JavaScript scripts via `node`.
 */
export class JavaScriptScriptExecutor implements ScriptExecutor {
  async execute(
    ...
    logger?.debug(`Script: executing 'node ${scriptPath}' (timeout: ${timeout}ms)`);

    return spawnScript('node', [scriptPath], {
